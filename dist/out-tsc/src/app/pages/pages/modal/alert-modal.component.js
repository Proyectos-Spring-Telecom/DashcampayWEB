...
let AlertModalComponent = class AlertModalComponent {
    get iconD() {
        const t = (this.state?.type ?? 'info');
        return this.iconPath[t];
    }
    constructor(alerts, router, cdr) {
        this.alerts = alerts;
        this.router = router;
        this.cdr = cdr;
        this.state = null;
    ...
    onKey(e) {
        ...
        if (e.key === 'Enter' && !e.shiftKey && !e.ctrlKey && !e.altKey && !e.metaKey && !e.isComposing) {
            if (this.state && !this.state.showCancel) {
                e.preventDefault();
                this.confirm();
                return;
            }
    ...
    finalizeClose() {
        ...
        this.alerts._resolve(this._lastResult);
        this._lastResult = null;
        this.state = null;
    ...
    close(reason) {
        ...
        if (this.closeFallback)
            clearTimeout(this.closeFallback);
        this.closeFallback = setTimeout(() => this.finalizeClose(), 260);
    }
    lockScroll() {
...
], AlertModalComponent.prototype, "onKey", null);
