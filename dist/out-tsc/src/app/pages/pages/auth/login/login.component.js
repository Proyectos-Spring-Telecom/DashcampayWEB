...
let LoginComponent = class LoginComponent {
    constructor(fb, auth, alerts, router, cdr, location) {
        this.fb = fb;
        this.auth = auth;
        this.alerts = alerts;
        this.router = router;
        ...
        this.auth.authenticate(credentials).subscribe({
            ...
            error: async (err) => {
                ...
                if (typeof err?.error === 'string') {
                    ...
                    try {
                        backendMsg = await err.error.text();
                    }
                    catch { }
                }
                ...
                if (!backendMsg)
                    backendMsg = err?.statusText || `HTTP ${err?.status || ''}`.trim();
                this.loading = false;
                this.textLogin = 'Iniciar Sesión';
                this.cdr.markForCheck();
                this.alerts.open({
    ...
    openOtpModal() {
        ...
        this.cdr.markForCheck();
        setTimeout(() => {
            const el = document.querySelector('#otp-modal .otp-email');
            el?.focus();
            el?.select?.();
        }, 0);
    }
    closeOtpModal(ev) {
        ev?.preventDefault();
        if (!this.otpOpen || this.otpClosing)
            return;
...
], LoginComponent.prototype, "otpInputs", void 0);
