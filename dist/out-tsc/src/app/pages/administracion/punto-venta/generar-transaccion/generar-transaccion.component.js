...
let GenerarTransaccionComponent = class GenerarTransaccionComponent {
    constructor(moneService, transaccionService, alerts, route, dialog, netpayService, authService) {
        ...
        this.swipeThreshold = -80; // Umbral para revelar el botón de eliminar
        this.swipeMaxDistance = -100; // Máxima distancia de deslizamiento
        this.listaMonederos = [];
        this.listaMonederosFiltrados = [];
    ...
    seleccionarTarjeta(tarjeta) {
        ...
        if (tokenCard) {
            ...
            if (this.direccionesDisponibles && this.direccionesDisponibles.length > 0) {
                const direccionEncontrada = this.direccionesDisponibles.find((dir) => dir.tokenCard === tokenCard);
    ...
    generarDeviceInformation() {
        ...
        return {
            ...
            httpBrowserLanguage: navigator.language || navigator.userLanguage || "es",
    ...
    obtenerTarjetas(customerId) {
        this.cargandoTarjetas = true;
        this.netpayService.obtenerTarjetasCliente(customerId).subscribe((response) => {
            this.cargandoTarjetas = false;
            ...
            if (response?.paymentSources && Array.isArray(response.paymentSources)) {
                this.tarjetasCliente = response.paymentSources.map((t) => ({
                    ...t,
                    _swipeX: 0,
                    _isSwiping: false
                }));
                ...
                this.tarjetasCliente = response.map((t) => ({
                    ...t,
                    _swipeX: 0,
                    _isSwiping: false
                }));
                ...
                this.tarjetasCliente = response.data.paymentSources.map((t) => ({
                    ...t,
                    _swipeX: 0,
                    _isSwiping: false
                }));
    ...
    loadNetpayScript() {
        return new Promise((resolve, reject) => {
            ...
            const existing = document.getElementById('netpay-sdk');
            if (existing) {
    ...
    crearClienteSiEsNecesario(token, deviceFingerPrint, deviceInformation, cvv, clienteInfo, referenceId) {
        ...
        if (customerId === null || customerId === undefined) {
            this.crearClienteNetpay(token, monedero, clienteInfo, cvv, referenceId).subscribe((response) => {
            ...
            this.actualizarTokenCliente(customerId, token, cvv, clienteInfo, referenceId).subscribe((response) => {
                this.cargando = false;
    ...
    actualizarTokenCliente(customerId, token, cvv, clienteInfo, referenceId) {
        let tokenData;
        if (clienteInfo) {
    ...
    crearClienteNetpay(token, monedero, clienteInfo, cvv, referenceId) {
    ...
    onSwipeStart(event, tarjeta, index) {
        ...
        if (!event.touches) {
            const onMouseMove = (e) => this.onSwipeMove(e, tarjeta);
            const onMouseUp = (e) => {
            ...
            const onTouchMove = (e) => this.onSwipeMove(e, tarjeta);
            const onTouchEnd = (e) => {
                this.onSwipeEnd(e, tarjeta);
    ...
    onSwipeMove(event, tarjeta) {
        if (!tarjeta._isSwiping)
            return;
        const touch = event.touches ? event.touches[0] : event;
    ...
    filtrarMonederos() {
        ...
        if (!q) {
            ...
            this.listaMonederosFiltrados = this.listaMonederos.filter((m) => `${m.numeroSerie || m.serie || ''} ${m.nombreCompletoPasajero || ''} ${m.clienteNombre || m.nombreCompletoCliente || ''}`
                .toLowerCase()
                .includes(q));
        }
    }
    ...
    agregar(payload) {
        this.cargando = true;
        this.transaccionService.agregarRecarga(payload).subscribe((_response) => {
            this.alerts.open({
                type: 'success',
                title: '¡Operación Exitosa!',
    ...
    getNombrePasajero(m) {
        ...
        const porPartes = ((m.pasajeroNombre || '') +
            ' ' +
    ...
    hasPasajero(m) {
        if (!m)
            return false;
        return !!((m.nombreCompletoPasajero && m.nombreCompletoPasajero.trim()) ||
            (m.pasajero && m.pasajero.trim()) ||
            ((m.pasajeroNombre || '') +
                (m.pasajeroApellidoPaterno || '') +
                (m.pasajeroApellidoMaterno || '')).trim());
    }
};
GenerarTransaccionComponent = __decorate([
    Component({
...
export { GenerarTransaccionComponent };
//# sourceMappingURL=generar-transaccion.component.js.map