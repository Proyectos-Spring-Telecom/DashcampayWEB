...
let AgregarTallerComponent = class AgregarTallerComponent {
    constructor(route, fb, talleresService, clientesService, activatedRoute, alerts, ngZone) {
        this.route = route;
        this.fb = fb;
        ...
        this.lat = 21.110778; // Misma coordenada que zona/ruta (Polanco/Cancún)
        this.lng = -86.762590;
        this.MAP_CENTER = { lat: 21.110778, lng: -86.762590 };
    }
    ngOnInit() {
    ...
    obtenerClientes() {
        this.clientesService.obtenerClientes().subscribe({
            next: (response) => {
                this.listaClientes = (response.data || response || []).map((item) => ({
                    ...item,
    ...
    initMap() {
        const el = document.getElementById('map-taller');
        if (!el)
            return;
    ...
    toggleMapType() {
        ...
        if (this.map) {
            this.map.setMapTypeId(this.mapType === 'satellite'
...
AgregarTallerComponent = __decorate([
    Component({
...
export { AgregarTallerComponent };
//# sourceMappingURL=agregar-taller.component.js.map