...
let DashboardComponent = class DashboardComponent {
    constructor(dashboardService, cdr) {
        ...
        this.ingresosHora = [];
        this.pagoDistrib = [];
        this.ascensosVsBoletos = [];
        this.pasajerosPorRuta = [];
        this.tiposPasajero = [];
        // Mapeo de colores para tipos de pasajero
        this.coloresTiposPasajero = {
            'estandar': '#008ffb',
            'estudiantes': '#f44336',
            'embarazadas': '#f44336',
            'discapacidad': '#ffc107' // Amarillo
        };
        this.histPuntualidad = [];
        this.mapLayers = [{
                type: 'area',
                data: 'world'
            }];
        ...
        this.dashboardService.obtenerMetricas(this.filtroSeleccionado.valor).subscribe({
            next: (data) => {
                ...
                if (data && data.top5Rutas && Array.isArray(data.top5Rutas)) {
                    ...
                    this.topRutas = data.top5Rutas.map((ruta) => ({
                ...
                if (data && data.graficaAscensosVsBoletos && Array.isArray(data.graficaAscensosVsBoletos)) {
                    this.ascensosVsBoletos = data.graficaAscensosVsBoletos.map((item) => ({
                ...
                if (data && data.pasajerosPorRutaTipo && Array.isArray(data.pasajerosPorRutaTipo)) {
                    ...
                    const tiposUnicosMap = new Map();
                    data.pasajerosPorRutaTipo.forEach((item) => {
                    ...
                    data.pasajerosPorRutaTipo.forEach((item) => {
                        ...
                        if (!rutasMap.has(rutaKey)) {
                            const rutaInicial = { ruta: rutaKey };
                ...
                if (data && Array.isArray(data.alertas)) {
                    ...
                    const toSeveridad = (s) => {
                        ...
                        if (v.includes('high') || v.includes('alta') || v.includes('crit'))
                            return 'sev-high';
                        if (v.includes('low') || v.includes('baja'))
                            return 'sev-low';
                        if (v.includes('med') || v.includes('media') || v.includes('warn'))
                            return 'sev-med';
                        return 'sev-med';
                    };
                    this.alertasValidadores = data.alertas.map((a) => {
    ...
    simularDatos() {
        ...
        const ingresos = horas.map(h => Math.max(0, 2500 + Math.sin((h + 2) / 24 * Math.PI * 2) * 1500 + (h < 5 ? -2000 : 0)));
        ...
        const buildFranja = () => rutas.reduce((acc, r) => (acc[r] = Math.round(800 + Math.random() * 200), acc), {});
