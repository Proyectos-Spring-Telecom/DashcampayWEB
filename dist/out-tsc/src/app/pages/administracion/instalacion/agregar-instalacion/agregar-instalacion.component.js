...
import { catchError, debounceTime, distinctUntilChanged, finalize, forkJoin, of, shareReplay } from 'rxjs';
var EstadoComponente;
(function (EstadoComponente) {
    EstadoComponente[EstadoComponente["INACTIVO"] = 0] = "INACTIVO";
    EstadoComponente[EstadoComponente["DISPONIBLE"] = 1] = "DISPONIBLE";
    EstadoComponente[EstadoComponente["ASIGNADO"] = 2] = "ASIGNADO";
    EstadoComponente[EstadoComponente["EN_MANTENIMIENTO"] = 3] = "EN_MANTENIMIENTO";
    EstadoComponente[EstadoComponente["DANADO"] = 4] = "DANADO";
    EstadoComponente[EstadoComponente["RETIRADO"] = 5] = "RETIRADO";
})(EstadoComponente || (EstadoComponente = {}));
let AgregarInstalacionComponent = class AgregarInstalacionComponent {
    get isAdmin() {
        return this.idRolUser === 1;
    }
    constructor(fb, instService, activatedRouted, route, dispoService, blueVoService, vehiService, clieService, users, cdr, alerts) {
        this.fb = fb;
        ...
        this.loading = false;
        this.title = 'Agregar Instalación';
        this.loadingDependientes = false;
        this.listaClientes = [];
        this.listaValidadores = [];
        this.listaContadores = [];
        this.listaVehiculos = [];
        this.placeholderVehiculo = []; // Para guardar el vehículo placeholder con cantidadPuertas
        ...
        // ---------- Varios ----------
        this.compareId = (a, b) => a != null && b != null && Number(a) === Number(b);
        this.trackId = (_, item) => Number(item?.id);
    ...
    get idContadoresArray() {
        return this.instalacionesForm.get('idContadores');
    }
    agregarContador() {
    ...
    obtenerContadoresDisponibles(index) {
        ...
        if (!this.idInstalacion) {
            contadoresFiltrados = contadoresFiltrados.filter((contador) => {
        ...
        return contadoresFiltrados.filter(contador => !contadoresSeleccionados.includes(Number(contador.id)));
    }
    estaAsignado(item) {
        if (!item)
            return false;
    ...
    puedeAgregarContador() {
        ...
        const vehiculoSeleccionado = this.listaVehiculos.find((v) => {
        ...
        const cantidadPuertas = Number(vehiculoSeleccionado?.cantidadPuertas ??
    ...
    desactivarCamposDependientes(disabled) {
        if (!this.instalacionesForm)
            return;
        const opts = { emitEvent: false };
        const idValidador = this.instalacionesForm.get('idValidador');
        const idContadores = this.instalacionesForm.get('idContadores');
    ...
    limpiarDependientes() {
        const opts = { emitEvent: false };
        this.instalacionesForm.patchValue({ idValidador: null, idVehiculo: null }, opts);
    ...
    suscribirCambioCliente() {
        this.instalacionesForm
            ...
            if (this.lastLoadedCliente === id)
                return;
            this.cargarListasPorCliente(id, false);
        });
    }
    suscribirCambioEquipos() {
        // Validador
        this.instalacionesForm
            .get('idValidador')
            ?.valueChanges.subscribe(async (nuevo) => {
            if (this.bootstrapping || !this.idInstalacion)
                return; // solo en edición
        ...
        this.idContadoresArray.valueChanges.subscribe(async (valores) => {
            if (this.bootstrapping || !this.idInstalacion)
                return; // solo en edición
    ...
    async cerrarModalValidador(cancelar) {
    ...
    async cerrarModalContador(cancelar) {
    ...
    confirmarModalContador() {
        ...
        if (this.pendingContadorIndex !== null) {
            ...
            if (idContadorAnterior != null) {
                ...
                const indexExistente = this.contadoresAnteriores.findIndex(c => c.idContador === idContadorAnterior);
                if (indexExistente >= 0) {
    ...
    cargarListasPorCliente(idCliente, applyPending) {
        ...
        const esError = (r) => r && typeof r === 'object' && '__error' in r;
        forkJoin({
            ...
            next: async (resp) => {
                const errores = [];
                // Normalizadores
                const mapValidador = (x) => ({
                    ...x,
                    id: Number(this.pickId(x, [
                        'id',
                        'idValidador',
                        'IdValidador',
                        'IDValidador'
                ...
                const mapVehiculo = (x) => ({
                    ...x,
                    id: Number(this.pickId(x, ['id', 'idVehiculo', 'IdVehiculo', 'IDVehiculo'])),
                    placa: x.placa ?? x.placaVehiculo ?? x.placavehiculo ?? '',
                    numeroEconomico: x.numeroEconomico ?? x.numeroEconomicoVehiculo ?? '',
                    cantidadPuertas: this.toNumOrNull(x.cantidadPuertas ??
                        x.cantidadpuertas ??
                        x.CantidadPuertas ??
                        null)
                });
                ...
                if (esError(resp.dispositivos)) {
                    ...
                    devs = this.ensureArray(resp?.dispositivos ?? resp?.data?.dispositivos ?? resp?.data).map(mapValidador);
                }
                if (esError(resp.bluevox)) {
                    errores.push(await this.getErrorMessage(resp.bluevox.__error));
                }
                else {
                    bvx = this.ensureArray(resp?.bluevox ?? resp?.data?.bluevox ?? resp?.data).map(mapContador);
                }
                if (esError(resp.vehiculos)) {
                    errores.push(await this.getErrorMessage(resp.vehiculos.__error));
                }
                else {
                    vehs = this.ensureArray(resp?.vehiculos ?? resp?.data?.vehiculos ?? resp?.data).map(mapVehiculo);
                }
                // Asegurar opción seleccionada aunque no esté en lista
                const sidDev = this.pendingSelecciones?.idValidador == null
                    ? null
                ...
                const sidBvx = this.pendingSelecciones?.idContador == null
                    ? null
                ...
                const sidVeh = this.pendingSelecciones?.idVehiculo == null
                    ? null
                ...
                if (sidVeh != null && !vehs.some((x) => Number(x.id) === sidVeh)) {
                    ...
                    const placeholderVeh = this.placeholderVehiculo.find((v) => Number(v.id) === Number(sidVeh));
                    vehs.unshift({
                        id: sidVeh,
                ...
                if (this.placeholderVehiculo.length > 0 && vehs.length > 0) {
                    ...
                    const vehIndex = vehs.findIndex((v) => Number(v.id) === Number(placeholderVeh.id));
    ...
    obtenerInstalacion() {
        ...
        this.instService.obtenerInstalacion(this.idInstalacion).subscribe({
            next: async (response) => {
                ...
                // IDs robustos
                const idClienteSrv = this.toNumOrNull(raw.idCliente ??
                    raw.idcliente ??
                    raw?.idCliente2?.id ??
                    raw?.cliente?.id ??
                    raw?.IdCliente ??
                    raw?.IDCliente);
                const estatus = this.toNumOrNull(raw.estatus ?? raw.Estatus ?? raw.status) ?? 1;
                const idValidador = this.toNumOrNull(raw.idValidador ??
                    raw.idvalidador ??
                    raw.idValidadores ??
                    raw.idvalidadores ??
                    raw?.dispositivos?.id ??
                ...
                if (raw.idContadores && Array.isArray(raw.idContadores)) {
                    idContadores = raw.idContadores.map((id) => this.toNumOrNull(id)).filter((id) => id != null);
                }
                else {
                    const idContador = this.toNumOrNull(raw.idContador ??
                        raw.idcontador ??
                        raw?.blueVoxs?.id ??
                        raw?.contador?.id ??
                        raw?.IdContador ??
                ...
                const idVehiculo = this.toNumOrNull(raw.idVehiculo ??
                    raw.idvehiculo ??
                    raw?.vehiculos?.id ??
                    raw?.vehiculo?.id ??
                    raw?.IdVehiculo ??
                ...
                const contadoresArray = this.instalacionesForm.get('idContadores');
                ...
                const placeholderValidador = idValidador
                    ? [
                        {
                            id: idValidador,
                            numeroSerie: (raw.numeroSerieValidadores ??
                                raw.numeroSerie ??
                                raw.numeroSerieDispositivo ??
                                '').toString(),
                            marca: (raw.marcaValidadores ??
                                raw.marcaValidador ??
                                raw.marca ??
                                '').toString(),
                            modelo: (raw.modeloValidadores ??
                                raw.modeloValidador ??
                                raw.modelo ??
                                '').toString()
                        }
                    ...
                    ? [
                        {
                            ...
                            numeroEconomico: (raw.numeroEconomicoVehiculo ??
                                raw.numeroEconomico ??
                                '').toString(),
                            cantidadPuertas: this.toNumOrNull(raw.cantidadPuertasVehiculo ??
                                raw.cantidadPuertas ??
                                raw.cantidadpuertas ??
                                raw.CantidadPuertas ??
                ...
                const contadoresArrayEdit = this.instalacionesForm.get('idContadores');
                contadoresArrayEdit.controls.forEach((control) => {
                    control.enable(opts);
                });
                // Pedir listas reales y fusionar
                const idClienteParaServicios = this.toNumOrNull(this.instalacionesForm.get('idCliente')?.value) ??
                    idCliente;
                const dispositivos$ = this.dispoService
                ...
                forkJoin({ dispositivos: dispositivos$, bluevox: bluevox$ }).subscribe({
                    next: async (resp) => {
                        ...
                        const contadoresArrayFork = this.instalacionesForm.get('idContadores');
    ...
    async agregar() {
        ...
        if (!raw.idValidador)
            camposFaltantes.push(etiquetas.idValidador);
        if (!raw.idVehiculo)
            camposFaltantes.push(etiquetas.idVehiculo);
        if (!raw.idCliente)
            camposFaltantes.push(etiquetas.idCliente);
        // Validar que haya al menos un contador seleccionado
        const idContadores = Array.isArray(raw.idContadores)
            ? raw.idContadores.filter((id) => id != null && id !== '').map((id) => this.toNumOrNull(id)).filter((id) => id != null)
            : [];
        if (idContadores.length === 0) {
    ...
    async actualizar() {
        ...
        const _estatusValAnt = this.estatusValidadorAnterior ??
            this.lastSubmittedMeta.estatusValidadorAnterior;
        const _comentVal = this.comentariosValidador ??
            this.lastSubmittedMeta.comentariosValidador ??
            null;
        const _contadoresAnteriores = this.contadoresAnteriores.length > 0
            ? this.contadoresAnteriores
            : this.lastSubmittedMeta.contadoresAnteriores;
        const _comentCon = this.comentariosContador ??
    ...
    async mostrarErroresServiciosSecuencial(mensajes) {
    ...
    ejecutarServiciosEquiposPorCliente(idCliente) {
        if (!idCliente)
            return;
        const dispositivos$ = this.dispoService
            .obtenerDispositivosByCliente(idCliente)
        ...
        forkJoin({ dispositivos: dispositivos$, bluevox: bluevox$ }).subscribe({
            next: async (resp) => {
                ...
                if (esError(resp.dispositivos)) {
                    ...
                    let validadores = this.ensureArray(resp.dispositivos ?? resp?.data?.dispositivos ?? resp?.data).map(mapValidador);
                    // Si estamos en modo agregar (no editar), filtrar los que contengan "Asignado"
                    if (!this.idInstalacion) {
                        validadores = validadores.filter((validador) => {
                            const numeroSerie = (validador.numeroSerie || '').toString().toLowerCase();
                            return !numeroSerie.includes('asignado');
                        });
                    }
                ...
                if (esError(resp.bluevox)) {
                    ...
                    let contadores = this.ensureArray(resp.bluevox ?? resp?.data?.bluevox ?? resp?.data).map(mapContador);
                    // Si estamos en modo agregar (no editar), filtrar los que contengan "Asignado"
                    if (!this.idInstalacion) {
                ...
                const opts = { emitEvent: false };
                this.instalacionesForm.get('idValidador')?.enable(opts);
                const idContadoresArray = this.instalacionesForm.get('idContadores');
    ...
    mergeUniqueByIdPreferFilled(primary = [], secondary = [], fields = ['numeroSerie', 'marca', 'modelo']) {
        const score = (o) => fields.reduce((s, f) => s + (o && typeof o[f] === 'string' && o[f].trim() ? 1 : 0), 0);
        const map = new Map();
        const put = (o) => {
            if (!o)
                return;
...
AgregarInstalacionComponent = __decorate([
    Component({
...
export { AgregarInstalacionComponent };
//# sourceMappingURL=agregar-instalacion.component.js.map