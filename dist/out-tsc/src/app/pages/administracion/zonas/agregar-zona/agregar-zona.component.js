var AgregarZonaComponent_1;
import { __decorate } from "tslib";
import { Component } from '@angular/core';
import { UntypedFormControl, Validators } from '@angular/forms';
import { fadeInRight400ms } from "../../../../../@vex/animations/fade-in-right.animation";
let AgregarZonaComponent = class AgregarZonaComponent {
    static { AgregarZonaComponent_1 = this; }
    get isAdmin() { return this.idRolUser === 1; }
    constructor(fb, zonService, activatedRouted, route, clieService, users, alerts) {
        this.fb = fb;
    ...
    ngAfterViewInit() {
        this.loadGoogleMaps()
            ...
            if (!window.google?.maps?.drawing) {
                ...
                this.showMapsErrorOverlay('No se cargó la librería de dibujo. Revisa que el script tenga &libraries=drawing.', 'Error de configuración');
                return;
            }
            this.initDrawing();
            this.addDrawControl();
            ...
            .catch((err) => {
            this.showMapsErrorOverlay('No se pudo cargar Google Maps', err?.message || 'Error desconocido');
    ...
    obtenerZona() {
        this.zonService.obtenerZona(this.idZona).subscribe({
            next: (response) => {
                ...
                const idCliSrv = Number(data?.idCliente ??
                    data?.idCliente2?.id ??
                    null);
                const gxAny = data?.geocerca ??
                    data?.poligono ??
                    data?.polygon ??
                    data?.coordenadas ??
                    null;
    ...
    buildFeatureCollectionFromForm() {
        let path = [];
        if (this.polygon) {
            path = this.polygon
                .getPath()
        ...
        const cleaned = path
            .map((p) => ({ lat: Number(p.lat), lng: Number(p.lng) }))
            .filter((p) => Number.isFinite(p.lat) && Number.isFinite(p.lng));
        const ring = cleaned.map((p) => [p.lng, p.lat]);
        if (ring.length >= 3) {
            const first = ring[0];
    ...
    loadGoogleMaps() {
        ...
        const existing = document.querySelector('script[data-gmaps="js"]') ||
            Array.from(document.getElementsByTagName('script')).find((s) => s.src.includes('maps.googleapis.com/maps/api/js'));
        if (existing) {
        ...
        if (!AgregarZonaComponent_1.mapsLoading) {
            AgregarZonaComponent_1.mapsLoading = this.loadGoogleMapsWithDrawing();
        }
        return AgregarZonaComponent_1.mapsLoading;
    }
    loadGoogleMapsWithDrawing() {
        return new Promise((resolve, reject) => {
    ...
    addDrawControl() {
        ...
        btn.onclick = () => {
            ...
            this.zonasForm.get('geocerca')?.markAsDirty();
            if (this.drawingManager && window.google?.maps?.drawing) {
    ...
    showMapsErrorOverlay(title, detail) {
        const el = this.getMapElement();
        if (!el)
            return;
        el.innerHTML = `
...
AgregarZonaComponent = AgregarZonaComponent_1 = __decorate([
    Component({
...
export { AgregarZonaComponent };
//# sourceMappingURL=agregar-zona.component.js.map