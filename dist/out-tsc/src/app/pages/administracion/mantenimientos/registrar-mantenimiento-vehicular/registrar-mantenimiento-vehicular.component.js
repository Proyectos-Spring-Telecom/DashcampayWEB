...
let RegistrarMantenimientoVehicularComponent = class RegistrarMantenimientoVehicularComponent {
    constructor(route, fb, mantenimientosService, activatedRoute, alerts) {
        ...
        this.loading = false;
        this.title = 'Registrar Mantenimiento Vehicular';
    ...
    obtenerReferencias() {
        this.mantenimientosService.obtenerReferenciasServicio().subscribe({
            next: (response) => {
                this.listaReferencias = (response.data || response || []).map((item) => ({
                    ...item,
                    id: Number(item?.id ?? item?.Id ?? item?.ID)
                }));
            },
    ...
    obtenerTalleres() {
        this.mantenimientosService.obtenerTalleres().subscribe({
            next: (response) => {
                this.listaTalleres = (response.data || response || []).map((item) => {
    ...
    onNotaServicioFileSelected(event) {
    ...
    onNotaServicioDragOver(event) {
        event.preventDefault();
        event.stopPropagation();
        this.notaServicioDragging = true;
    }
    onNotaServicioDragLeave(event) {
        event.preventDefault();
        event.stopPropagation();
        this.notaServicioDragging = false;
    }
    onNotaServicioDrop(event) {
        event.preventDefault();
        event.stopPropagation();
        this.notaServicioDragging = false;
...
__decorate([
    ViewChild('notaServicioInput')
], RegistrarMantenimientoVehicularComponent.prototype, "notaServicioInput", void 0);
RegistrarMantenimientoVehicularComponent = __decorate([
    Component({
...
export { RegistrarMantenimientoVehicularComponent };
//# sourceMappingURL=registrar-mantenimiento-vehicular.component.js.map