...
let ListaClientesComponent = class ListaClientesComponent {
    constructor(cliService, alerts, router, // para navegar
    route) {
    ...
    setupDataSource() {
        ...
        this.listaClientes = new CustomStore({
            ...
            load: async (loadOptions) => {
                ...
                try {
                    const response = await lastValueFrom(this.cliService.obtenerClientesData(page, take));
                    this.loading = false;
                    ...
                    const dataTransformada = (Array.isArray(response?.data) ? response.data : []).map((item) => {
...
], ListaClientesComponent.prototype, "dataGrid", void 0);
